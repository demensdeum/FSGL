...
static const auto OGLRendererScreenWidth = 1024;
static const auto OGLRendererScreenHeight = 576;
...
GLint OGLRenderer::common_get_shader_program(const char *vertex_shader_source, const char *fragment_shader_source) {
...
void OGLRenderer::removeObject(shared_ptr<FSGLObject> object) {
...
void OGLRenderer::removeObjectAtIndex(shared_ptr<FSGLObject> object, int index) {
...
SDL_Window* OGLRenderer::initialize() {
    ...
    glViewport(0, 0, OGLRendererScreenWidth, OGLRendererScreenHeight);
    ...
    glm::mat4 projectionMatrix = glm::perspective(45.0f, float(float(OGLRendererScreenWidth) / float(OGLRendererScreenHeight)), 0.001f, 200.0f);
...
void OGLRenderer::addObject(shared_ptr<FSGLObject> object) {
	if (object->flag2D) {
...
void OGLRenderer::render() {
	...
	if (object.get() == nullptr) {
		...
		cout << "OGLRenderer cannot render empty object" << endl;
...
void OGLRenderer::stop() {

}

shared_ptr<FSGLObject> OGLRenderer::getObjectWithID(string id) {
...
void OGLRenderer::renderObject(shared_ptr<FSGLObject> object) {
	...
	if (material == NULL) {
            ...
            throw logic_error("OGLRenderer: cannot load material");
        ...
        if (surface == NULL) {
            ...
            cout << "OGLRenderer: cannot load texture " << material->texturePath->c_str() << endl;
	
		throw logic_error("OGLRenderer: cannot load texture");
